...
function log(msg, level)
{
    level = level || 1;
    var req = new XMLHttpRequest();
    var url = "http://192.168.2.10/" + "&PlexConnectATVLogLevel=" + level.toString() + "&PlexConnectLog=" + encodeURIComponent(msg);
    req.open('GET', url, true);
    req.send();
};
...
atv.onAppEntry = function()
{
    ...
    if (parseFloat(firmVer) >= 5.1)
    {
        ...
        var url = "http://192.168.2.10/&PlexConnect=Discover&PlexConnectUDID="+atv.device.udid
        var req = new XMLHttpRequest();
        req.open('GET', url, false);
        req.send();
        
        // load main page
        atv.loadURL("http://192.168.2.10/PlexConnect.xml&PlexConnectUDID=" + atv.device.udid);
    }
    else
    {
        var xmlstr =
